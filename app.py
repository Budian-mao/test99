...
def insert2():
    ...
    studynumber = request.form.get("studynumber")
    score = request.form.get("score")

    value = (id, lessonname, teachername, studynumber, score)
    insert_sql = '''INSERT INTO classes(id, lessonname, teachername, studynumber, score) values (%s,%s,%s,%s,%s)'''
    cursor = get_cursor()  # 打开数据库
    cursor.execute(insert_sql, value)  # 执行sql语句
    msg="添加成功！"
    return msg
...
@app.route('/delete2/<int:teachername>')
def delete2(teachername):
    cursor = get_cursor()  # 打开数据库
    cursor.execute("delete from classes where teachername = %s", (teachername))  # 删除某一行值
    msg="删除成功！"
    return msg
...
def lessons(id):
    ...
    lists= cursor.fetchall()
    return render_template("lessons.html", id=id, lists=lists)
...
def update2():
    ...
    value = (id, lessonname, teachername, studynumber, score)
    cursor = get_cursor()  # 打开数据库
    cursor.execute("update classes set id = %s,lessonname = %s,studynumber =%s,score = %s  where teachername = %s", value)
    msg="编译成功！"
    return msg
